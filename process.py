...
MAX_WORKERS = min(4, os.cpu_count())  # Reduced workers to prevent bandwidth issues
# ======================================================================================
...
def process_single_take(action_name, take_number):
    ...
    try:
        ...
        if not os.path.exists(input_bag_1):
            return f"ERROR: Missing bag file: {input_bag_1}"
        if not os.path.exists(input_bag_2):
            return f"ERROR: Missing bag file: {input_bag_2}"
        ...
        try:
            while True:
                ...
                # Save individual frames only (no video processing)
        ...
        return f"ERROR processing '{action_name}' take {take_number:02d}: {str(e)}"
...
def main():
    ...
    action_names = ["Approaching", "Grasping", "Transporting", "Releasing", "Idle"]  # Test with all actions
    ...
    # Print the status message from each worker to see any errors
    for res in results:
        print(res)
